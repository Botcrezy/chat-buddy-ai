...
import { Search, UserPlus, Download, Upload, Users, Phone, MessageSquare, Star, User } from "lucide-react";
          ...
          <Dialog open={showAdd} onOpenChange={setShowAdd}>
            ...
            <DialogContent dir="rtl">
              ...
              <div className="space-y-3 mt-2">
                ...
                <div className="space-y-1"><Label>الاسم</Label><Input value={newContact.name} onChange={(e) => setNewContact({ ...newContact, name: e.target.value })} placeholder="أحمد" /></div>
                ...
                <div className="space-y-1"><Label>ملاحظات</Label><Textarea value={newContact.notes} onChange={(e) => setNewContact({ ...newContact, notes: e.target.value })} placeholder="ملاحظات..." /></div>
              ...
              {filtered.map((contact) => (
                <div key={contact.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors border">
                  <div className="h-11 w-11 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                    {contact.category === "vip" ? <Star className="h-5 w-5 text-yellow-500" /> : <User className="h-5 w-5 text-primary" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-sm">{contact.name || "بدون اسم"}</span>
